...
def kernel_Wendland_periodic(x1, x2, sigmasq=1.0, l=1.0, D=1, q=0, L=1.0):
  ...
  if q==0:
    ...
    K += jnp.maximum(0, 1-tau_warped)**(j+2) * ( (j**2+4*j+3)*tau_warped**2 +
                                (3*j+6)*tau_warped + 3 ) / 3
  elif q==3:
    K += jnp.maximum(0, 1-tau_warped)**(j+3) * ( (j**3+9*j**2+23*j+15)*tau_warped**3 +
                            (6*j**2+36*j+45)*tau_warped**2 + (15*j+45)*tau_warped  + 15 ) / 15
  else:
    raise ValueError('Not implemented q={}'.format(q))
  return sigmasq * K
