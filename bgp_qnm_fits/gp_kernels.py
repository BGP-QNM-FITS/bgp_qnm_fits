...
def kernel_Wendland(x1, x2, sigmasq=1.0, l=1.0, D=1, q=0):
    ...
    if q == 0:
        ...
        K += jnp.maximum(0, 1 - tau) ** (j + 2) * ((j**2 + 4 * j + 3) * tau**2 + (3 * j + 6) * tau + 3) / 3
    elif q == 3:
        K += (
            jnp.maximum(0, 1 - tau) ** (j + 3)
            * ((j**3 + 9 * j**2 + 23 * j + 15) * tau**3 + (6 * j**2 + 36 * j + 45) * tau**2 + (15 * j + 45) * tau + 15)
            / 15
        )
    else:
...
def kernel_Wendland_periodic(x1, x2, sigmasq=1.0, l=1.0, D=1, q=0, L=1.0):
    l, D, q = float(l), int(D), int(q)
    K = jnp.zeros((len(x1), len(x2)))
    j = int(D / 2) + q + 1
    tau = jnp.abs(x1[:, jnp.newaxis] - x2[jnp.newaxis, :]) / jnp.abs(l)
    tau_warped = 4 * jnp.sin(jnp.pi * tau / L) ** 2
    if q == 0:
        K += jnp.maximum(0, 1 - tau_warped) ** (j)
    elif q == 1:
        K += jnp.maximum(0, 1 - tau_warped) ** (j + 1) * ((j + 1) * tau_warped + 1)
    elif q == 2:
        K += (
            jnp.maximum(0, 1 - tau_warped) ** (j + 2)
            * ((j**2 + 4 * j + 3) * tau_warped**2 + (3 * j + 6) * tau_warped + 3)
            / 3
        )
    elif q == 3:
        K += (
            jnp.maximum(0, 1 - tau_warped) ** (j + 3)
            * (
                (j**3 + 9 * j**2 + 23 * j + 15) * tau_warped**3
                + (6 * j**2 + 36 * j + 45) * tau_warped**2
                + (15 * j + 45) * tau_warped
                + 15
            )
            / 15
...
def kernel_GPC(analysis_times, **kwargs):
    ...
    return (
        (
            ...
            # squared_exp_element(t1, t2, kwargs["period"]) #** kwargs["a"]
            + periodic_kernel(t1, t2, kwargs["length_scale_2"], kwargs["period_2"]) * (1 - kwargs["a"])
            # + kernel_Wendland_periodic(t1, t2, sigmasq=1.0, l=kwargs["period_2"], q=3, L=kwargs["length_scale_2"]) * (1 - kwargs["a"])
        )
        * new_func(
            t1,
            kwargs["length_scale"],
...
def compute_kernel_matrix_test(t1, t2, hyperparams, kernel, regularization_factor=1e2):
    return kernel(t1, t2, **hyperparams) + jnp.eye(len(t1)) * (hyperparams["A_min_reg"] * regularization_factor) ** 2
