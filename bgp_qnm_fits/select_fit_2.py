...
class BGP_select2(Base_BGP_fit):
    """
    ...
    def get_fit_at_t0(self, t0):
        ...
        while True:
            ...
            for candidate_mode in candidate_modes_considered:
                ...
                full_frequencies = jnp.array([self._get_frequency(mode, chif_ref, Mf_ref) for mode in try_modes])
                full_frequency_derivatives = jnp.array([self._get_domega_dchif(mode, chif_ref, Mf_ref) for mode in try_modes])
                full_mixing_coefficients = jnp.array([self._get_mixing(mode, sph_mode, chif_ref) for sph_mode in self.spherical_modes for mode in try_modes]).reshape(len(self.spherical_modes), len(try_modes))
                full_mixing_derivatives = jnp.array([self._get_dmu_dchif(mode, sph_mode, chif_ref) for sph_mode in self.spherical_modes for mode in try_modes]).reshape(len(self.spherical_modes), len(try_modes))
